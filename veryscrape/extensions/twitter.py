...
class Twitter(SearchClient):
    ...
    async def filter_stream(self, track=None, topic=None, duration=3600, use_proxy=False):
        ...
        async with await self.post('statuses/filter.json', {}, oauth=1, params={'langauge': 'en', 'track': track},
                                   use_proxy={'speed': 100, 'https': 1, 'post': 1} if use_proxy else None) as raw:
            if raw.status == 420:
